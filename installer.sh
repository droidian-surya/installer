...
$ARIA2BIN -o "$(dirname "$0")"/cache/ $DTWRP -q || echo "Downloading TWRP failed please try again or fix" && exit
...
$ARIA2BIN -o "$(dirname "$0")"/cache/boot.img $DVBMETA -q || echo "Downloading VBMETA failed please try again or fix" && exit
clear; echo; echo; echo; echo; echo; echo -e "Flashing VBMETA"; echo -e "   [===              ]" # 15 spaces
$FASTBOOTBIN flash vbmeta "$(dirname $0)"/cache/vbmeta.img
clear; echo; echo; echo; echo; echo; echo -e "Rebooting to recovery.."; echo -e "   [===              ]" # 15 spaces
$FASTBOOTBIN reboot recovery
$ARIA2BIN -o "$(dirname "$0")"/cache/boot.img $DROOTFS -q || echo "Downloading ROOTFS failed please try again or fix" && exit